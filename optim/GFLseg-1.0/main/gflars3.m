function res = gflars(Y,weight,k,option)
...
M = zeros(n,n-1);
for its = 2:n
    M(its,1:(its-1)) = weights(its);
end
Xbar = (weights * weights' - diag(ones(n,1)))*M;
Ybar =  Y - weights * (sum(weights.*Y))/(sum(weights.^2));
% Initialize cHat = X'*Y
